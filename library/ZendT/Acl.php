<?php
   ...
   class ZendT_Acl {
       ...
       public function add($role, $resource, $allowed = true) {
           $this->_privileges[] = array('role' => $role
              , 'resource' => $resource
              , 'allowed' => $allowed);
           return $this;
       }
       ...
       public function isAllowed($name, $resourceBase = null, $role = null) {
           ...
           $isAllow = $this->_isAllowed($rowSession->getRole(), $resourceBase);
           ...
           if (!$isAllow && is_array($roles)) {
               ...
               foreach ($roles as $role) {
                   $isAllow = $this->_isAllowed($role, $resourceBase);
       ...
       public function restriction($name, $resourceBase = null) {
           ...
           return $this->_isAllowed($rowSession->getRole(), $resourceBase);
