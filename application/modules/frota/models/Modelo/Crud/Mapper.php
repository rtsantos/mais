 ...
 */
class Frota_Model_Modelo_Crud_Mapper extends ZendT_Db_Mapper
{
    protected $_required = array('id','id_marca','descricao','status','id_empresa');
    protected $_model = 'Frota_Model_Modelo_Table';
    public static $table = 'mais.fr_modelo';
    /**
     *
     * @var Frota_Model_Modelo_Mapper
     */
    ...
    public function getValueOld(){
        if (!$this->_dataOld instanceof Frota_Model_Modelo_Mapper){
            $this->_dataOld = new Frota_Model_Modelo_Mapper();
    ...
    public function getReferenceMap(){
        return array(
                ...
                'id_marca' => array(
                    'mapper' => 'Frota_DataView_Marca_MapperView',
                    'column' => 'id'
                ));
    }
    /**
  ...
  array (
    ...
    'column' => 'id_modelo',
    'message' => 'Necessário seleção Modelo',
  ),
);
    }
     ...
     *
     * @param string $value
     * @return Frota_Model_Modelo_Crud_Mapper
     */
    ...
    /**
     * Retorna os dados da coluna id_marca
     *
     * @return string
     */
    public function getIdMarca($instance=false){
        if ($instance && !is_object($this->_data['id_marca'])){
            $this->setIdMarca('',array('required'=>false));
        }
        return $this->_data['id_marca'];
    }
    /**
     * Seta o valor da coluna id_marca
     *
     * @param string $value
     * @return Frota_Model_Modelo_Crud_Mapper
     */
    public function setIdMarca($value,$options=array('required'=>true)){        
        $this->_data['id_marca'] = new ZendT_Type_Number($value,array('numDecimal'=>null));
         if ($options['db'])
            $this->_data['id_marca']->setValueFromDb($value);
                    
        if (!$options['db']){
            
         if ($options['required'])
            $this->isRequired($value,'id_marca');
