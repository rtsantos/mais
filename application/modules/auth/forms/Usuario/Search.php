<?php
    class Auth_Form_Usuario_Search extends ZendT_Form {
         ...
         */
         ...
         $this->addElement($element);
             ...
             */
                ...
                $element->setValue('Auth_Model_Usuario_Mapper');
                $this->addElement($element);
            ...
            /**
             * Pesquisa na coluna idusuarioresp
             */
            
                $element = new ZendT_Form_Element_Hidden('idusuariorespfield');
                $element->setBelongsTo('filter[idusuarioresp][field]');
                $element->setValue('usuario.idusuarioresp');
                $this->addElement($element);

                $element = new ZendT_Form_Element_Hidden('idusuariorespmapper');
                $element->setBelongsTo('filter[idusuarioresp][mapper]');
                $element->setValue('Auth_Model_Usuario_Mapper');
                $this->addElement($element);

                $element = new ZendT_Form_Element_SelectSqlOperation('idusuariorespop');
                $element->setBelongsTo('filter[idusuarioresp][op]');
                $element->setLabel($translate->_('operacao').':');
                $this->addElement($element);
                ...
                $nameElement = $element->getName();
                $element->setLabel($translate->_('valor').':');
                $element->setBelongsTo('filter[idusuarioresp][value]');
                $this->addElement($element);


                $this->addDisplayGroup(
                    array (
                        'idusuariorespop',$nameElement
                    ),'idusuarioresp.idusuariorespop'
                );

                $fields = $this->getDisplayGroup('idusuarioresp.idusuariorespop');
                $fields->addDecorator('Fieldset');
                $fields->setLegend($translate->_('usuario.idusuarioresp') . ':')->setAttrib('id', 'idusuarioresp-fieldset');
            /**
