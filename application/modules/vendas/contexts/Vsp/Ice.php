<?php
   ...
   class Vendas_Context_Vsp_Ice {
       ...
       protected function _pedido($data) {
           ...
           $_where->addFilter('cv_pedido.dt_emis', array($ini, $fim), 'BETWEEN');

           if (!$_pedido->exists($_where)) {
       ...
       public function _vistoria($data) {
           ...
           $_vistoria->setIdPedido($data->id_pedido);
           $_vistoria->setIdVeiculo($data->id_veiculo);
           $exists = $_vistoria->exists();
           ...
           if ($_vistoria->getLaudo(true)->toPhp() == '') {
       ...
       protected function _consultorias() {
           $params = array();
           $params['header']['Authorization'] = 'bearer ' . $this->_token;
           $params['get']['Placa'] = '';
