<?php
   ...
   class Vendas_Model_Pagamento_Mapper extends Vendas_Model_Pagamento_Crud_Mapper {
       ...
       public function _beforeSave() {
           ...
           if ($this->getVlrTotal(true)->toPhp() == '') {
               $vlrTotal = 0;
               $_itens = new Vendas_DataView_ItemPedido_MapperView();
               $_itens->setIdPedido($this->getIdPedido());
               $_itens->findAll(null, '*');
               while ($_itens->fetch()) {
                   $vlrTotal+= $_itens->getVlrFinal()->toPhp();
               }
               $this->setVlrTotal($vlrTotal);
           }

           if ($this->getVlrAPagar(true)->toPhp() == '') {
               $this->setVlrAPagar($this->getVlrTotal());
           }

           if ($this->getVlrPago(true)->toPhp() == '') {
               $this->setVlrPago($this->getVlrAPagar());
           }

           if ($this->getVlrParc(true)->toPhp() == '') {
               $this->setVlrParc($this->getVlrPago());
           }

           if ($this->getIdParcela(true)->toPhp() == '') {
               $_parcela = new Vendas_DataView_Parcela_MapperView();
               $_parcela->setIdEmpresa($_pedido->getIdEmpresa())
                     ->setQtd(1)
                     ->setStatus('A')
                     ->retrieve();
               $this->setIdParcela($_parcela->getId());
           }

           if ($this->getDtVencParc(true)->toPhp() == '') {
               $this->setDtVencParc($_pedido->getDtEmis());
           }
       }
           ...
           while ($row = $rows->getRow()) {
               ...
               for ($index = 0; $index < $qtd; $index++) {
                   ...
                   $_lancamento->newRow()
                         ...
                         ->setDescricao('VENDA DE PEDIDO')
                         ->setObservacao('PAGAMENTO ' . ($index + 1) . " de " . $qtd . '. PEDIDO: ' . $row['numero_pedido'])
       ...
       public function cancelar($idPedido = false) {
           ...
           while($_itemLanc->fetch()){
               $_lancamento->setId($_itemLanc->getIdLancamento())
                           ->retrieve()
                           ->cancelar();
           }
           ...
           while($_pagtoLanc->fetch()){
               $_lancamento->setId($_pagtoLanc->getIdLancamento())
                           ->retrieve()
                           ->cancelar();
           }
