...
collect=collect.substr(0,len);return collect;};input+='';pad_string=pad_string!==undefined?pad_string:' ';if(pad_type!='0'&&pad_type!='1'&&pad_type!='2'){pad_type='1';}
if((pad_to_go=pad_length-input.length)>0){if(pad_type=='0'){input=str_pad_repeater(pad_string,pad_to_go)+input;}else if(pad_type=='1'){input=input+str_pad_repeater(pad_string,pad_to_go);}else if(pad_type=='2'){half=str_pad_repeater(pad_string,Math.ceil(pad_to_go/2));input=half+input+half;input=input.substr(0,pad_length);}}
return input;}
