...
(function ($) {
    ...
    $.widget('ta.' + widget_name, {
        ...
        _create: function () {
            ...
            //$("body").append(self.getScriptElement());
            $("body").append(self.getHtmlElement());
            ...
            $("body").append(this.getScriptChange());
            $("body").ready(function () {
        ...
        addElement: function () {
            ...
            if (this.element_qtd >= 1) {
                htmlSeparacao = "";
                var classes = 'ui-button item ui-state-default';
                if ($(this.elements_sel).first().hasClass('required')) {
                    //classes = classes + ' required';
                }
                htmlSeparacao = htmlSeparacao + "<div id='" + this.menu_div_sep + this.id + '-' + this.element_qtd + "' class='" + classes + "'></div>";
            }
        ...
        updateButtons: function () {
            ...
            if (operador == this.getOperatorByNumber()) {
                var size = $(this.elements_sel).size();
                if (size < 2) {
                ...
                if (size == 1) {
                    $("#" + this.btn_add + this.id).addClass('border');
                } else {
                    $("#" + this.btn_add + this.id).removeClass('border');
                }
            }
        },
        ...
        getHtmlMenu: function () {
            var htmlMenu = '';
            htmlMenu = htmlMenu + '<a id="' + this.menu_href + this.id + '" href="#' + this.menu_div + this.id + '" class="ui-button ui-state-default item border">'
            htmlMenu = htmlMenu + '    <span id="' + this.menu_icon + this.id + '" class="ui-icon ui-icon-triangle-1-s">';
            htmlMenu = htmlMenu + '    </span>';
            htmlMenu = htmlMenu + '    <span id="' + this.menu_label + this.id + '" class="ui-text"></span>';
            htmlMenu = htmlMenu + '    <input id="' + this.menu_value + this.id + '" type="hidden"/>';
            htmlMenu = htmlMenu + '</a>';
            htmlMenu = htmlMenu + '<div id="' + this.menu_div + this.id + '" class="hidden" style="display:inline-block;">';
            htmlMenu = htmlMenu + '    <ul class="dropdown-menu" role="' + this.menu_href + this.id + '">';
            var onClick = '';
            ...
            jQuery.each(this.operadores, function (index) {
                htmlMenu = htmlMenu + '<li><a href="#" value="' + $(this)[0] + '"><label>' + $(this)[1] + '</label></a></li>';
            });
            htmlMenu = htmlMenu + '    </ul>';
            htmlMenu = htmlMenu + '</div>';
            htmlMenu = htmlMenu + '<script>$("#' + this.menu_div + this.id + '").find("ul li a").click(function(){' + onClick + '});</script>';
            htmlMenu = htmlMenu + '<script>$("#' + this.menu_div + this.id + '").find("ul li a").first().click();</script>';
            return htmlMenu;
        },
        getHtmlButtons: function () {
            var htmlButtons = '';
            htmlButtons = htmlButtons + '<div id="' + this.btn_group + this.id + '" style="margin-left:5px;float:left;" class="ui-form-group">';
            htmlButtons = htmlButtons + '  <span id="' + this.btn_del + this.id + '" class="ui-button ui-state-default item first" title="Remover">';
            htmlButtons = htmlButtons + '      <span class="ui-button-icon-primary ui-icon ui-icon-minus">';
            htmlButtons = htmlButtons + '      </span>';
            htmlButtons = htmlButtons + '  </span>';
            htmlButtons = htmlButtons + '  <span id="' + this.btn_add + this.id + '" class="ui-button ui-state-default item" title="Adicionar">';
